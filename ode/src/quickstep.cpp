        ...
        /*************************************************************/
        {
          ...
          for (int i=0; i<m;J_ptr+=12,JiM_ptr+=12, i++) {
            ...
            if (b2 >= 0){
              ...
              JiMratio_ptr[0] = 1.0;
              for (int j=0;j<3;j++) {
                if  (!_dequal(JiM_ptr[  j], 0.0)) JiMratio_ptr[0] = std::max(JiMratio_ptr[0],JiM_ptr[6+j]/JiM_ptr[  j]);
                if  (!_dequal(JiM_ptr[3+j], 0.0)) JiMratio_ptr[0] = std::max(JiMratio_ptr[0],JiM_ptr[9+j]/JiM_ptr[3+j]);
                if  (!_dequal(JiM_ptr[6+j], 0.0)) JiMratio_ptr[0] = std::max(JiMratio_ptr[0],JiM_ptr[  j]/JiM_ptr[6+j]);
                if  (!_dequal(JiM_ptr[9+j], 0.0)) JiMratio_ptr[0] = std::max(JiMratio_ptr[0],JiM_ptr[3+j]/JiM_ptr[9+j]);
              }
            }
          }
        }
